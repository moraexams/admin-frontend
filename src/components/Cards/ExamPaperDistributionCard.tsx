				...
				<table className="table-fixed border-separate border-spacing-1 text-black dark:text-white">
					<tbody>
						<tr>
							...
							<td>:</td>
							<td>
								<strong>{centre.bus_route}</strong>
							</td>
						</tr>
						<tr>
							...
							<td>:</td>
							<td>
								<strong>{centre.bus_departure_time}</strong>
							</td>
						</tr>
						<tr>
							<td>Arrival Time</td>
							<td>:</td>
							<td>
								<strong>{centre.bus_arrival_time}</strong>
							</td>
						</tr>
						<tr>
							...
							<td>:</td>
							<td>
								<strong>{centre.travel_duration}</strong>
							</td>
						</tr>
						<tr>
							...
							<td>:</td>
							<td>
								<strong>{centre.substitude_times}</strong>
							</td>
						</tr>
					</tbody>
				</table>
				...
				{centre.counts && centre.counts.length > 0 ? (
					<table className="min-w-full divide-y divide-gray-200 mt-2 dark:bg-slate-800 text-black dark:text-white">
						...
						<tbody className="bg-white divide-y divide-slate-400">
							{sortByKey(centre.counts, "subject").map((paper: Count) => (
								<tr
									key={paper.code.concat(paper.medium)}
...
const ExamPaperDistributionCard = ({
	districtData,
	district,
	centre,
}: {
		...
		<div className="flex-wrap justify-center">
			...
			<div>
				...
				{district && centre === -1 && filteredCentres.length > 0 ? (
					...
					filteredCentres
						.filter((c) => c.id === centre)
						.map((centre) => <CentreCard key={centre.id} centre={centre} />)
				) : (
