...
const VerifyStudent = () => {
    ...
    const handleVerifyStudent = async () => {
        if (student) {
            verifyStudent(Number(student.index_no)).then(() => {
        ...
        <DefaultLayout>
            ...
            <div className="w-full min-h-[75vh] rounded-lg bg-white px-8 py-6 mt-6 dark:bg-boxdark md:px-17.5 md:py-8">
                <div className='flex items-center justify-center gap-4 flex-col md:flex-row'>
                    ...
                    />
                    <button onClick={searchStudent} className="block rounded border border-primary bg-primary p-3 text-center font-medium text-white transition hover:bg-opacity-90">
                        Search Student
                    </button>
                </div>
                        ...
                        <div><table className="table-fixed border-separate border-spacing-2 text-black dark:text-white">
                            <tbody>
                                ...
                                <tr>
                                    <td>Stream</td>
                                    <td>:</td>
                                    <td><strong>{student.stream?.name}</strong></td>
                                </tr>
                                <tr>
                                    <td>Medium</td>
                                    <td>:</td>
                                    <td><strong>{student.medium}</strong></td>
                                </tr>
                                <tr>
                                    <td>Rank District</td>
                                    <td>:</td>
                                    <td><strong>{student.rank_district?.name}</strong></td>
                                </tr>
                                <tr>
                                    <td>Exam District</td>
                                    <td>:</td>
                                    <td><strong>{student.exam_district?.name}</strong></td>
                                </tr>
                                <tr>
                                    <td>Exam Centre</td>
                                    <td>:</td>
                                    <td><strong>{student.exam_centre?.name}</strong></td>
                                </tr>
                                <tr>
                                    <td>Registered By</td>
                                    <td>:</td>
                                    <td><strong>{student.registered_by?.username}</strong></td>
                                </tr>
                            </tbody>
                        </table>
                            {!student.checked_by_id && showVerify && (<div className='flex items-center justify-center gap-4 flex-col md:flex-row'>
                                <button onClick={handleVerifyStudent} className="block rounded border border-success bg-success p-3 text-center font-medium text-white transition hover:bg-opacity-90">
                                    Verify Student
                                </button>
                            </div>)}

                        </div>
                    ) : (<div className="text-center text-danger font-bold">Student Not Found</div>)}
